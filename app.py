...
# ------------------- YOUTUBE INFO FETCH (ROBUST VERSION) -------------------
def get_info(url):
    try:
        import yt_dlp
        ydl_opts = {
            "quiet": True, 
            "skip_download": True,
            "extract_flat": "in_playlist" if "playlist" in url.lower() else False,  # Flat for playlists to avoid errors
            "ignoreerrors": True
        }
        ...
        st.error(f"Preview failed: {str(e)}")  # Graceful error
        return None
...
with options_col1:
    mode = st.selectbox("Mode", ["Video", "Playlist"], help="Choose download type")  # Removed Audio to avoid merge issues
with options_col2:
    video_quality = st.selectbox("Video Quality", ["144p", "240p", "360p", "480p", "720p", "1080p", "1440p (2K)", "2160p (4K)"])
with options_col3:
    # Placeholder column since audio removed
    st.write("")
    ...
    if info:
        ...
        if mode == "Playlist" and "entries" in info:
            ...
            for i, entry in enumerate(entries[:5], 1):
                title = entry.get('title') if isinstance(entry, dict) else "N/A"
                st.write(f"{i}. {title}")
            if len(entries) > 5:
                st.write(f"... and {len(entries) - 5} more")
        st.markdown('</div>', unsafe_allow_html=True)
    else:
        st.warning("Could not fetch preview. URL may be invalid or YouTube is updating. Try download directly.")
...
if st.button("Start Download", key="download_btn"):
    if not url:
        ...
        # No FFmpeg check needed now, since no merging
        ...
        with st.spinner("Processing your request..."):
            ...
            download_type = "video"  # Always video now
